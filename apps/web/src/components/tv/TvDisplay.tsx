    ...
    <div className={isDark ? "dark" : ""}>
      <div className="flex h-screen w-full flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white overflow-hidden relative font-sans transition-colors duration-500">
        ...
        <main className="relative z-10 flex flex-1 p-6 lg:p-12 gap-8 lg:gap-12 items-stretch h-[80vh]">
          ...
          <div className={`flex-[2] flex flex-col rounded-[3rem] p-8 shadow-xl dark:shadow-2xl border relative overflow-hidden backdrop-blur-2xl transition-colors duration-500 ${hasEmergency
            ...
            {activeTokens.length === 0 ? (
              ...
              <div className={`flex-1 grid gap-4 lg:gap-6 ${activeTokens.length === 1 ? 'grid-cols-1'
                ...
                {activeTokens.map((item: any, idx: number) => {
                  ...
                  return (
                    <div key={idx} className={`flex flex-col items-center justify-center rounded-[2rem] border transition-all duration-300 p-4 ${pulseScale ? 'scale-105' : 'scale-100'} ${isEmergency
              ...
              {queueData.nextTokens.length > 0 ? (
                queueData.nextTokens.slice(0, 5).map((token, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={`flex items-center justify-center p-[2.5vh] rounded-2xl border shadow-sm dark:shadow-none transition-colors duration-500 ${isEmergency
